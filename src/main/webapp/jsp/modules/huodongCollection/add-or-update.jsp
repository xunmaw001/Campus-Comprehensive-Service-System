    ...
    <div id="content">
        ...
        <div class="container mt-0">
            <div class="row breadcrumb-bar">
                <div class="col-md-6">
                    <h3 class="block-title">编辑活动收藏</h3>
                </div>
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">编辑活动收藏</li>
                    </ol>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                ...
                <div class="col-md-12">
                    <div class="widget-area-2 lochana-box-shadow">
                        <h3 class="widget-title">活动收藏信息</h3>
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 当前表的字段 -->
                                ...
                                <input id="yonghuId" name="yonghuId" type="hidden">
                                    <div class="form-group col-md-6 huodongCollectionTypesDiv">
                                        <label>类型</label>
                                        <select style="width: 450px" id="huodongCollectionTypesSelect" name="huodongCollectionTypes" class="form-control">
                                        </select>
                                    </div>
...
<script>
    ...
    // 下拉框数组
        <!-- 当前表的下拉框数组 -->
    var huodongCollectionTypesOptions = [];
        <!-- 级联表的下拉框数组 -->
    var huodongOptions = [];
    var yonghuOptions = [];
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            ...
            httpJson("huodongCollection/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
                    window.sessionStorage.removeItem('addhuodongCollection');
    ...
    // 查询列表
        ...
        function huodongCollectionTypesSelect() {
            //填充下拉框选项
            http("dictionary/page?page=1&limit=100&sort=&order=&dicCode=huodong_collection_types", "GET", {}, (res) => {
                if(res.code == 0){
                    huodongCollectionTypesOptions = res.data.list;
                }
            });
        }
        <!-- 查询级联表的所有列表 -->
    ...
    // 初始化下拉框
    <!-- 初始化当前表的下拉框 -->
        function initializationHuodongcollectiontypesSelect(){
            var huodongCollectionTypesSelect = document.getElementById('huodongCollectionTypesSelect');
            if(huodongCollectionTypesSelect != null && huodongCollectionTypesOptions != null  && huodongCollectionTypesOptions.length > 0 ){
                for (var i = 0; i < huodongCollectionTypesOptions.length; i++) {
                    huodongCollectionTypesSelect.add(new Option(huodongCollectionTypesOptions[i].indexName,huodongCollectionTypesOptions[i].codeIndex));
                }
            }
        }
    ...
    function setSelectOption() {
        ...
        if(huodongCollectionTypesSelect != null && huodongCollectionTypesOptions != null  && huodongCollectionTypesOptions.length > 0 ) {
            for (var i = 0; i < huodongCollectionTypesOptions.length; i++) {
                if (huodongCollectionTypesOptions[i].codeIndex == ruleForm.huodongCollectionTypes) {//下拉框value对比,如果一致就赋值汉字
                        huodongCollectionTypesSelect.options[i].selected = true;
                }
            }
        }
        <!--  级联表的下拉框回显  -->
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                huodongId: "required",
                yonghuId: "required",
                huodongCollectionTypes: "required",
            },
            messages: {
                huodongId: "活动不能为空",
                yonghuId: "用户不能为空",
                huodongCollectionTypes: "类型不能为空",
            }
        }).form();
    }
    ...
    // 获取当前详情
    function getDetails() {
        var addhuodongCollection = window.sessionStorage.getItem("addhuodongCollection");
        if (addhuodongCollection != null && addhuodongCollection != "" && addhuodongCollection != "null") {
            //注册表单验证
            $(validform());
            ...
            window.sessionStorage.removeItem('updateId');
            http("huodongCollection/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
    ...
    $(document).ready(function () {
        ...
        // 初始化下拉框
            <!--  初始化当前表的下拉框  -->
            initializationHuodongcollectiontypesSelect();
            <!--  初始化级联表的下拉框  -->
