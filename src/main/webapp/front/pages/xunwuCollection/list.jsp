            ...
            <div class="ng-inmod-ini">
                    <div class="ng-fault ng-container">
                        <div class="ng-inmod-the ng-container">
                            ...
                            <span class="ng-inmod-the-cn" aos="fade-down">寻物收藏展示</span>
                    ...
                    <form class="layui-form" lay-filter="myForm">
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="xunwuName" id="xunwuName" placeholder="寻物名称" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                                                                                 
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              <label class="layui-form-label">寻物类型</label>
                              <div class="layui-input-inline">
                                  <select name="xunwuTypes" id="xunwuTypes" lay-filter="xunwuTypes">
                                      <option value="">--请选择--</option>
                                      <option v-for="item in xunwuTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                                  </select>
                              </div>
                          </div>
                ...
                <div class="ng-inside-box ng-fault ng-container" aos="fade-up">
                    <div class="show-reel">
                        <div v-for="(item,index) in dataList" v-bind:key="index" class="col-md-3 agile-gallery-grid">
                            <i @click="deleteXunwuCollection(item.id)" class="layui-icon" style="float:right;color:red;margin-top: -10px;font-size:20px">&#xe640;</i>
                            <div class="agile-gallery">
                                <a @click="jumpCheck('../xunwu/detail.jsp?id='+item.xunwuId , item.xunwuDelete == null?'':item.xunwuDelete , item.shangxiaTypes == null?'':item.shangxiaTypes)" class="lsb-preview" data-lsb-group="header">
                                    <img :src="item.xunwuPhoto?item.xunwuPhoto.split(',')[0]:''" style="height: 350px;width:100%;" />
                                    <div class="agileits-caption">
                                        <h4>{{item.xunwuName}}</h4>
                                        <p v-if="item.xunwuNewMoney != null" >￥{{item.xunwuNewMoney}}</p>
                                    </div>
                                </a>
                            </div>
                        </div>
...
<script>
    var vue = new Vue({
        ...
        data: {
            ...
            dataList: [],//数据
        // 当前表字典表数据
            xunwuCollectionTypesList : [],
        // 级联表字典表数据
            xunwuTypesList : [],
            xunwuZhuangtaiTypesList : [],
            xunwuCollectionTypes: 0,//类型
            limit: 8,
        ...
        methods: {
            ...
            ,deleteXunwuCollection(id){
                layui.layer.confirm('确认要删除这个收藏吗？', {
                    btn : [ '确定', '取消' ]//按钮
                }, function(index) {
                    var paramArray = [];
                    paramArray.push(id);
                    layui.http.requestJson('xunwuCollection/delete', 'post',paramArray, function (res) {
                        if (res.code == 0) {
                            layer.msg('删除成功', {
                                time: 2000,
                                icon: 6
    ...
    // 当前表的字典表
        ...
        function xunwuCollectionTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=xunwu_collection_types", 'get', {}, function (res) {
                if(res.code == 0){
                    vue.xunwuCollectionTypesList = res.data.list;
                }
            });
        }
    ...
    // 级联表的字段数据
        ...
        function xunwuZhuangtaiTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=xunwu_zhuangtai_types", 'get', {}, function (res) {
                if(res.code == 0){
        ...
        function pageList() {
            ...
            if (jquery('#xunwuName').val() != null && jquery('#xunwuName').val() != "" && jquery('#xunwuName').val() != "null" ) {
                param['xunwuName'] = jquery('#xunwuName').val();
            }
                                                                                 
            var xunwuTypes = $("#xunwuTypes").find("option:selected").val();
            if (xunwuTypes != null && xunwuTypes != "" && xunwuTypes != "null") {
                            param['xunwuTypes'] = xunwuTypes;
                        }
                     
            var xunwuZhuangtaiTypes = $("#xunwuZhuangtaiTypes").find("option:selected").val();
            if (xunwuZhuangtaiTypes != null && xunwuZhuangtaiTypes != "" && xunwuZhuangtaiTypes != "null") {
                            param['xunwuZhuangtaiTypes'] = xunwuZhuangtaiTypes;
                        }
                                                                                                                                            
            // 当前表
                                                
            //收藏只能查类型为1的
            param['xunwuCollectionTypes'] = 1;

            // 获取列表数据
            http.request('xunwuCollection/page', 'get', param, function(res) {
