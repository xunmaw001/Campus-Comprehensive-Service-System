            ...
            <div class="ng-inmod-ini">
                    <div class="ng-fault ng-container">
                        <div class="ng-inmod-the ng-container">
                            ...
                            <span class="ng-inmod-the-cn" aos="fade-down">活动收藏展示</span>
                    ...
                    <form class="layui-form" lay-filter="myForm">
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="huodongUuidNumber" id="huodongUuidNumber" placeholder="活动编号" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="huodongName" id="huodongName" placeholder="活动名称" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <select name="huodongTypes" id="huodongTypes" lay-filter="huodongTypes">
                                      <option value="">--请选择--</option>
                                      <option v-for="item in huodongTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                                  </select>
                              </div>
                          </div>
                ...
                <div class="ng-inside-box ng-fault ng-container" aos="fade-up">
                    <div class="show-reel">
                        <div v-for="(item,index) in dataList" v-bind:key="index" class="col-md-3 agile-gallery-grid">
                            <i @click="deleteHuodongCollection(item.id)" class="layui-icon" style="float:right;color:red;margin-top: -10px;font-size:20px">&#xe640;</i>
                            <div class="agile-gallery">
                                <a @click="jumpCheck('../huodong/detail.jsp?id='+item.huodongId , item.huodongDelete == null?'':item.huodongDelete , item.shangxiaTypes == null?'':item.shangxiaTypes)" class="lsb-preview" data-lsb-group="header">
                                    <img :src="item.huodongPhoto?item.huodongPhoto.split(',')[0]:''" style="height: 350px;width:100%;" />
                                    <div class="agileits-caption">
                                        <h4>{{item.huodongName}}</h4>
                                        <p v-if="item.huodongNewMoney != null" >￥{{item.huodongNewMoney}}</p>
                                    </div>
                                </a>
                            </div>
                        </div>
...
<script>
    var vue = new Vue({
        ...
        data: {
            ...
            dataList: [],//数据
        // 当前表字典表数据
            huodongCollectionTypesList : [],
        // 级联表字典表数据
            huodongTypesList : [],
            huodongCollectionTypes: 0,//类型
            limit: 8,
        ...
        methods: {
            ...
            ,deleteHuodongCollection(id){
                layui.layer.confirm('确认要删除这个收藏吗？', {
                    btn : [ '确定', '取消' ]//按钮
                }, function(index) {
                    var paramArray = [];
                    paramArray.push(id);
                    layui.http.requestJson('huodongCollection/delete', 'post',paramArray, function (res) {
                        if (res.code == 0) {
                            layer.msg('删除成功', {
                                time: 2000,
                                icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate', 'tinymce'], function() {
        ...
        // 查询当前字典表相关
            // 类型的查询和初始化
        huodongCollectionTypesSelect();
    // 当前表的字典表
        // 类型的查询
        function huodongCollectionTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=huodong_collection_types", 'get', {}, function (res) {
                if(res.code == 0){
                    vue.huodongCollectionTypesList = res.data.list;
                }
            });
        }
    ...
    // 级联表的字段数据
        ...
        function huodongTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=huodong_types", 'get', {}, function (res) {
                if(res.code == 0){
        ...
        function pageList() {
            ...
            if (jquery('#huodongUuidNumber').val() != null && jquery('#huodongUuidNumber').val() != "" && jquery('#huodongUuidNumber').val() != "null" ) {
                param['huodongUuidNumber'] = jquery('#huodongUuidNumber').val();
            }
                     
            if (jquery('#huodongName').val() != null && jquery('#huodongName').val() != "" && jquery('#huodongName').val() != "null" ) {
                param['huodongName'] = jquery('#huodongName').val();
            }
                                         
            var huodongTypes = $("#huodongTypes").find("option:selected").val();
            if (huodongTypes != null && huodongTypes != "" && huodongTypes != "null") {
                            param['huodongTypes'] = huodongTypes;
                        }
                                                                                                                                            
            // 当前表
                                                
            //收藏只能查类型为1的
            param['huodongCollectionTypes'] = 1;

            // 获取列表数据
            http.request('huodongCollection/page', 'get', param, function(res) {
