            ...
            <div class="ng-inmod-ini">
                    <div class="ng-fault ng-container">
                        <div class="ng-inmod-the ng-container">
                            ...
                            <span class="ng-inmod-the-cn" aos="fade-down">心里咨询师收藏展示</span>
                    ...
                    <form class="layui-form" lay-filter="myForm">
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="xinlizixunshiName" id="xinlizixunshiName" placeholder="咨询师姓名" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                ...
                <div class="ng-inside-box ng-fault ng-container" aos="fade-up">
                    <div class="show-reel">
                        <div v-for="(item,index) in dataList" v-bind:key="index" class="col-md-3 agile-gallery-grid">
                            <i @click="deleteXinlizixunshiCollection(item.id)" class="layui-icon" style="float:right;color:red;margin-top: -10px;font-size:20px">&#xe640;</i>
                            <div class="agile-gallery">
                                <a @click="jumpCheck('../xinlizixunshi/detail.jsp?id='+item.xinlizixunshiId , item.xinlizixunshiDelete == null?'':item.xinlizixunshiDelete , item.shangxiaTypes == null?'':item.shangxiaTypes)" class="lsb-preview" data-lsb-group="header">
                                    <img :src="item.xinlizixunshiPhoto?item.xinlizixunshiPhoto.split(',')[0]:''" style="height: 350px;width:100%;" />
                                    <div class="agileits-caption">
                                        <h4>{{item.xinlizixunshiName}}</h4>
                                        <p v-if="item.xinlizixunshiNewMoney != null" >￥{{item.xinlizixunshiNewMoney}}</p>
                                    </div>
                                </a>
                            </div>
                        </div>
...
<script>
    var vue = new Vue({
        ...
        data: {
            ...
            dataList: [],//数据
        // 当前表字典表数据
            xinlizixunshiCollectionTypesList : [],
        // 级联表字典表数据
            xinlizixunshiCollectionTypes: 0,//类型
            limit: 8,
        ...
        methods: {
            ...
            ,deleteXinlizixunshiCollection(id){
                layui.layer.confirm('确认要删除这个收藏吗？', {
                    btn : [ '确定', '取消' ]//按钮
                }, function(index) {
                    var paramArray = [];
                    paramArray.push(id);
                    layui.http.requestJson('xinlizixunshiCollection/delete', 'post',paramArray, function (res) {
                        if (res.code == 0) {
                            layer.msg('删除成功', {
                                time: 2000,
                                icon: 6
    ...
    // 当前表的字典表
        ...
        function xinlizixunshiCollectionTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=xinlizixunshi_collection_types", 'get', {}, function (res) {
                if(res.code == 0){
                    vue.xinlizixunshiCollectionTypesList = res.data.list;
                }
            });
        }
    ...
    // 级联表的字段数据
        ...
        function pageList() {
            ...
            if (jquery('#xinlizixunshiName').val() != null && jquery('#xinlizixunshiName').val() != "" && jquery('#xinlizixunshiName').val() != "null" ) {
                param['xinlizixunshiName'] = jquery('#xinlizixunshiName').val();
            }
                                                                                                                        
            // 当前表
                                                
            //收藏只能查类型为1的
            param['xinlizixunshiCollectionTypes'] = 1;

            // 获取列表数据
            http.request('xinlizixunshiCollection/page', 'get', param, function(res) {
