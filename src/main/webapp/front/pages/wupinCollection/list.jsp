            ...
            <div class="ng-inmod-ini">
                    <div class="ng-fault ng-container">
                        <div class="ng-inmod-the ng-container">
                            ...
                            <span class="ng-inmod-the-cn" aos="fade-down">二手市场收藏展示</span>
                    ...
                    <form class="layui-form" lay-filter="myForm">
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="wupinName" id="wupinName" placeholder="物品名称" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <select name="wupinTypes" id="wupinTypes" lay-filter="wupinTypes">
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <select name="wupinMaichuTypes" id="wupinMaichuTypes" lay-filter="wupinMaichuTypes">
                                      <option value="">--请选择--</option>
                                      <option v-for="item in wupinMaichuTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                                  </select>
                              </div>
                          </div>
                ...
                <div class="ng-inside-box ng-fault ng-container" aos="fade-up">
                    <div class="show-reel">
                        <div v-for="(item,index) in dataList" v-bind:key="index" class="col-md-3 agile-gallery-grid">
                            <i @click="deleteWupinCollection(item.id)" class="layui-icon" style="float:right;color:red;margin-top: -10px;font-size:20px">&#xe640;</i>
                            <div class="agile-gallery">
                                <a @click="jumpCheck('../wupin/detail.jsp?id='+item.wupinId , item.wupinDelete == null?'':item.wupinDelete , item.shangxiaTypes == null?'':item.shangxiaTypes)" class="lsb-preview" data-lsb-group="header">
                                    <img :src="item.wupinPhoto?item.wupinPhoto.split(',')[0]:''" style="height: 350px;width:100%;" />
                                    <div class="agileits-caption">
                                        <h4>{{item.wupinName}}</h4>
                                        <p v-if="item.wupinNewMoney != null" >￥{{item.wupinNewMoney}}</p>
                                    </div>
                                </a>
                            </div>
                        </div>
...
<script>
    var vue = new Vue({
        ...
        data: {
            ...
            dataList: [],//数据
        // 当前表字典表数据
            wupinCollectionTypesList : [],
        // 级联表字典表数据
            wupinTypesList : [],
            wupinMaichuTypesList : [],
            wupinCollectionTypes: 0,//类型
            limit: 8,
        ...
        methods: {
            ...
            ,deleteWupinCollection(id){
                layui.layer.confirm('确认要删除这个收藏吗？', {
                    btn : [ '确定', '取消' ]//按钮
                }, function(index) {
                    var paramArray = [];
                    paramArray.push(id);
                    layui.http.requestJson('wupinCollection/delete', 'post',paramArray, function (res) {
                        if (res.code == 0) {
                            layer.msg('删除成功', {
                                time: 2000,
                                icon: 6
    ...
    // 当前表的字典表
        ...
        function wupinCollectionTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=wupin_collection_types", 'get', {}, function (res) {
                if(res.code == 0){
                    vue.wupinCollectionTypesList = res.data.list;
                }
            });
        }
    ...
    // 级联表的字段数据
        ...
        function wupinTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=wupin_types", 'get', {}, function (res) {
                if(res.code == 0){
        ...
        function wupinMaichuTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=wupin_maichu_types", 'get', {}, function (res) {
                if(res.code == 0){
                    vue.wupinMaichuTypesList = res.data.list;
                }
            });
        }
        ...
        function pageList() {
            ...
            if (jquery('#wupinName').val() != null && jquery('#wupinName').val() != "" && jquery('#wupinName').val() != "null" ) {
                param['wupinName'] = jquery('#wupinName').val();
            }
                                                                                                     
            var wupinTypes = $("#wupinTypes").find("option:selected").val();
            if (wupinTypes != null && wupinTypes != "" && wupinTypes != "null") {
                            param['wupinTypes'] = wupinTypes;
                        }
                                         
            var wupinMaichuTypes = $("#wupinMaichuTypes").find("option:selected").val();
            if (wupinMaichuTypes != null && wupinMaichuTypes != "" && wupinMaichuTypes != "null") {
                            param['wupinMaichuTypes'] = wupinMaichuTypes;
                        }
                                                                                                                        
            // 当前表
                                                
            //收藏只能查类型为1的
            param['wupinCollectionTypes'] = 1;

            // 获取列表数据
            http.request('wupinCollection/page', 'get', param, function(res) {
